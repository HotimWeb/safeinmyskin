emailjs.init("XBpGQiUTpFAbAfmeE"); // <-- Add your EmailJS user ID here
...
document.getElementById("contact-form").addEventListener("submit", function(event) {
    ...
    let selectedShare = [];
    document.querySelectorAll('input[name="share_methods"]:checked').forEach((checkbox) => {
        selectedShare.push(checkbox.value);
    });

    // Get selected checkboxes for "How would you like to get rewards"
    let selectedReward = [];
    document.querySelectorAll('input[name="reward_options"]:checked').forEach((checkbox) => {
        selectedReward.push(checkbox.value);
    });

    // Get selected checkboxes for "terms and conditions"
    let termsCondition = [];
    document.querySelectorAll('input[name="agree_terms"]:checked').forEach((checkbox) => {
        termsCondition.push(checkbox.value);
    });

    // Get selected checkboxes for "what best describes you"
    let bestDescribes = [];
    document.querySelectorAll('input[name="best_describe"]:checked').forEach((checkbox) => {
        bestDescribes.push(checkbox.value);
    });

    // Get selected radio buttons
    const selectedSellChoice = document.querySelector('input[name="sell_choice"]:checked');
    const selectedRecommed = document.querySelector('input[name="inte_rest"]:checked');
    const stockSkincare = document.querySelector('input[name="sims_someone"]:checked');
    ...
    emailjs.send("service_gf5quyb", "template_j3z16ff", {
